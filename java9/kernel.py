...
def get_error_lines(code, out, out_p):
    out_p_lines = []
    code_lines = code.split("\n")
    for line in code_lines:
        if out_p:
            current = out_p[:len(line)]
            out_p_lines.append(current)
            out_p = out_p[len(line) + 1:]
        else:
            break
    line_num = len(out_p_lines) - 1
    return code_lines[line_num], out_p_lines[line_num], line_num + 1
...
class JavaKernel(Kernel):
    ...
    def __init__(self, **kwargs):
        ...
        self._banner = None
        self.env = {
            "JAVA_9_HOME": os.environ['JAVA_9_HOME'],
...
#             )
    ...
    def _execute_java(self, code):
        ...
        interrupted = False
        try:
            output = self.javawrapper.run_command(code, timeout=None) #code.rstrip(), timeout=None)
            ...
            #self.javawrapper.child.sendintr()
            output = self.javawrapper.child.before + 'Incomplete Java expression. Restarting and clearing everything...' + "\r\n\r\n"
            self._start_java_repl()
    ...
    def do_execute_direct(self, code, silent=False):
        ...
        code_one_line = code.replace("\n", " ")
        interrupted, output = self._execute_java(code_one_line)
        ...
        output_pos = 0
        source_pos = 0
        ## remove echoed output
        # skip first line, end of input:
        output_pos = 1
        while output_pos < len(original_output):
            line = original_output[output_pos]
            if line.startswith("|    assigned to temporary variable"):
                pass
            elif line.startswith("|  Expression value is:"):
                pass
            elif line.startswith("|  Warning:"):
                next_line = original_output[output_pos + 1]
                if "not permitted in top-level declarations, ignored" in next_line:
                    output_pos += 3
                else:
                    new_output.append(line)
            elif code_one_line.strip() in line:
                pass # skip it
            elif re.match(r"\W*\^", line):
                l1, l2, line_num = get_error_lines(code, code_one_line, line[3:])
                new_output.append("|  Line number: %s" % line_num)
                new_output.append(l1)
                new_output.append(l2)
            else:
                new_output.append(line)
            output_pos += 1
